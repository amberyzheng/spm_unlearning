...
class DDPM(nn.Module):
    ...
    def forward(self, x, support, c, c_support, use_amp=False):
        ...
        support = normalize_to_neg_one_to_one(support)

        x_combined = torch.cat([x, support], dim=0)
        c_combined = torch.cat([c, c_support], dim=0)

        x_noised, t, noise = self.perturb(x_combined, t=None)

        support_mask = torch.cat([
            torch.zeros(x.shape[0], dtype=torch.bool, device=x.device),
            torch.ones(support.shape[0], dtype=torch.bool, device=x.device)
        ], dim=0)

        # 0 for conditional, 1 for unconditional
        mask = torch.bernoulli(torch.zeros_like(c_combined) + self.drop_prob).to(self.device)

        with autocast(enabled=use_amp):
            return self.loss(noise[~support_mask], self.nn_model(x_noised, t / self.n_T, c_combined, mask, support_mask=support_mask))
        
    
    def prepare_condition_for_sample(self, n_sample, cond, support, cond_support):
        B_support = support.shape[0]
        cond_all = torch.cat([cond, cond_support, cond, cond_support], dim=0)
        mask_all = torch.cat([
            torch.zeros(n_sample, device=self.device),
            torch.zeros(B_support, device=self.device),
            torch.ones(n_sample, device=self.device),
            torch.zeros(B_support, device=self.device)
        ], dim=0)
        model_args_all = (cond_all, mask_all)
        support_mask = torch.cat([
            torch.zeros(n_sample, dtype=torch.bool, device=self.device),
            torch.ones(B_support, dtype=torch.bool, device=self.device)
        ], dim=0)
        return model_args_all, support_mask
    ...
    def sample(self, 
        ...
        model_args_all, support_mask = self.prepare_condition_for_sample(n_sample, cond, support, cond_support)
        ...
        for i in tqdm(range(self.n_T, 0, -1), disable=notqdm):
            ...
            eps, _ = self.pred_eps_(
                torch.cat([x_i, support], dim=0),
                t_is, 
                model_args_all, 
                guide_w, 
                alpha, 
    ...
    def ddim_sample(self, 
        ...
        support = normalize_to_neg_one_to_one(support)
        model_args_all, support_mask = self.prepare_condition_for_sample(n_sample, cond, support, cond_support)
        B_total = support.shape[0] + n_sample
        ...
        for time, time_next in tqdm(time_pairs, disable=notqdm):
            t_is = torch.tensor([time / self.n_T]).to(self.device).repeat(B_total)

            z = torch.randn(n_sample, *size).to(self.device) if time_next > 0 else 0

            alpha = sche["alphabar_t"][time]
            eps, x0_t = self.pred_eps_(torch.cat([x_i, support], dim=0),
                                        t_is, 
                                        model_args_all, 
                                        guide_w, 
                                        alpha, 
    ...
    def pred_eps_(self, x, t, model_args, guide_w, alpha, use_amp, support_mask, clip_x=True):
        def pred_cfg_eps_double_batch():
            # double batch
            x_double = x.repeat(2, 1, 1, 1)
            support_mask_double = support_mask.repeat(2)
            t_double = t.repeat(2)

            with autocast(enabled=use_amp):
                eps = self.nn_model(x_double, t_double, *model_args, support_mask=support_mask_double).float()
        ...
        def pred_eps_from_x0(x0):
            return (x[~support_mask] - x0 * alpha.sqrt()) / (1 - alpha).sqrt()

        def pred_x0_from_eps(eps):
            return (x[~support_mask] - (1 - alpha).sqrt() * eps) / alpha.sqrt()
