...
class PairwiseRelationSpatial(nn.Module):
    def __init__(self, embed_dim_q, embed_dim_s, hidden_dim=None):
        super(PairwiseRelationSpatial, self).__init__()
    ...
    def forward(self, q, S, q_labels, S_labels):
        B_main, P, _ = q.shape
        B_support, P_S, _ = S.shape
 
        out = torch.zeros_like(q)
        q_trans = self.linear_q(q)
        S_trans = self.linear_s(S)
 
        # Group support patches by label
        support_map = {}
        for i, lab in enumerate(S_labels):
            lab = lab.item()
            if lab not in support_map:
                support_map[lab] = []
            support_map[lab].append(i)
 
        # Group query patches by integer label (supports drop label)
        query_map = {}
        for i, lab in enumerate(q_labels):
            lab = lab.item()
            if lab not in query_map:
                query_map[lab] = []
            query_map[lab].append(i)

        for label, q_inds in query_map.items():
            s_inds = support_map.get(label, [])
            # If there is no matching labeled support, use all support
            if not s_inds:
                if self.training:
                    continue
                else:
                    s_inds = list(range(B_support))

            q_l = q_trans[q_inds]  # (num_q, P, hidden_dim)
            S_l = S_trans[s_inds]  # (num_s, P_S, hidden_dim)

            q_exp = q_l.unsqueeze(2).unsqueeze(3)
            S_exp = S_l.unsqueeze(0).unsqueeze(0)
            h = F.relu(q_exp + S_exp + self.bias)
                
            scores = self.linear_out(h).squeeze(-1)
            weights = F.softmax(scores.view(len(q_inds), P, -1), dim=-1)
            weights = weights.view(len(q_inds), P, len(s_inds), P_S)

            S_expanded = S_l.unsqueeze(0).unsqueeze(0)
            weights = weights.unsqueeze(-1)
            out_label = (weights * S_expanded).sum(dim=(2, 3))
            out[q_inds] = out_label


        return out


class SemiParametricMiddleBlock(nn.Module):
    def __init__(self, n_channels, time_channels, attn_channels_per_head, dropout, pool_size=4):
        super().__init__()
        self.res = ResidualBlock(n_channels, n_channels, time_channels, dropout=dropout)
        self.pairwise_relation_expert_spatial = PairwiseRelationSpatial(
            n_channels, n_channels, hidden_dim=attn_channels_per_head
        )
        self.res_post = ResidualBlock(n_channels, n_channels, time_channels, dropout=dropout)
        self.pool_size = pool_size

    def forward(self, x, t, c, c_int, support_mask):

        x_feat = self.res(x, t, c)

        x_emb = x_feat[~support_mask]
        support_feat = x_feat[support_mask].detach()
        c_main = c[~support_mask]
        c_int_main = c_int[~support_mask] if c_int is not None else None
        c_int_support = c_int[support_mask] if c_int is not None else None
        
        # Transform to a lower resolution.
        q_repr = F.adaptive_avg_pool2d(x_emb, (self.pool_size, self.pool_size))   # (B_main, n_channels, pool_size, pool_size)
        S_repr = F.adaptive_avg_pool2d(support_feat, (self.pool_size, self.pool_size))  # (B_support, n_channels, pool_size, pool_size)
        B_main, C, _, _ = q_repr.shape
        B_support, _, _, _ = S_repr.shape
        q_repr_flat = q_repr.view(B_main, C, -1).permute(0, 2, 1)  # (B_main, P, n_channels) with P = pool_size^2
        S_repr_flat = S_repr.view(B_support, C, -1).permute(0, 2, 1)  # (B_support, P, n_channels)
        
        pairwise_out_flat = self.pairwise_relation_expert_spatial(q_repr_flat, S_repr_flat, c_int_main, c_int_support)  # (B_main, P, n_channels)
        fused_low_res = pairwise_out_flat.permute(0, 2, 1).view(B_main, C, self.pool_size, self.pool_size)
        # Upsample to match the original spatial size of x_emb.
        fused = F.interpolate(fused_low_res, size=x_emb.shape[2:], mode='nearest')
        
        t = t[~support_mask] if t.shape[0] > 1 else t
        out = self.res_post(fused, t, c_main)
...
class UNet(nn.Module):
    def __init__(self, image_shape = [3, 32, 32], n_channels = 128,
        ...
        * `n_classes` is the number of classes in the condition
        ...
        # Middle block
        self.middle = SemiParametricMiddleBlock(out_channels, time_channels, attn_channels_per_head, dropout)
    ...
    def forward(self, x, t, c, drop_mask, support_mask=None):
        """
        * `x` has shape `[batch_size, in_channels, height, width]`
        ...
        * `drop_mask` has shape `[batch_size]`
        ...
        for m in self.down:
            ...
            h.append(x[~support_mask])

        x = self.middle(x, t, c, c_int, support_mask).contiguous()
        t = t[~support_mask] if t.shape[0] > 1 else t
        c = c[~support_mask]
